...
├── contact/                # Contact form (client-side validated) + contact info
├── footer/
...
All page content is pulled from the `data/` files — components never hardcode copy. This means you can update almost everything about the site without touching component code.
...
- `resumePath` — path to your resume PDF (see step 4 below)
- `profileImage` — path to your photo (see step 3 below)
...
1. Add your photo to `public/images/profile.jpg` (or update the path in `data/personal.ts` → `profileImage` if you use a different filename)
2. Open `components/about/AboutFull.tsx` and replace the placeholder `<div>` block (marked with a comment) with:
   ```tsx
   <img
...
Add your resume PDF to `public/resume/Nazmul-Alam-Resume.pdf` (matching the path in `data/personal.ts` → `resumePath`, or update that path to match your filename). The "Download Resume" button on the About page will then work automatically.

### 5. Skills

Edit **`data/skills.ts`**:

- `skillCategories` — add/remove technologies per category; the UI adapts automatically
...
The contact form (`components/contact/ContactForm.tsx`) validates input client-side but does **not** send messages yet — the submit button is intentionally disabled after validation, with a `// TODO` comment marking where to wire up a real email service (e.g. Resend, EmailJS) or a FastAPI endpoint once one is deployed.
